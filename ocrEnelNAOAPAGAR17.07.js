  ...
  function dadosNota(pdfData) {
    ...
    const getTUSD0605 = (pdf) => {
      const table = pdf.tables.find((table) => {
        ...
        return valuesContent.some((row) => row.includes('USO SIST. DISTR (TUSD)'));
      });

      if (!table) return false;

      const valuesContent = table.valuesContent;
      const row = valuesContent.find((row) => row.includes('USO SIST. DISTR (TUSD)'));

      if (!row) {
        const fallbackRow = valuesContent.find((row) => row.includes('0605'));
